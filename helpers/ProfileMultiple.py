...
class ProfileMultiple:
    ...
    def export(self) -> None:
        ...
        for result in self.results:
            print(len(result))
            df = df._append({'con_label': result[0], 'default_CPU': result[1][0], 'default_GPU': result[1][1], 'ttgt_CPU': result[2][0], 'ttgt_GPU': result[2][1], 'tgett_CPU': result[3][0], 'tgett_GPU': result[3][1], 'gett_CPU': result[4][0], 'gett_GPU': result[4][1], 'default_patient_CPU': result[5][0], 'default_patient_GPU': result[5][1], 'cuquantum_CPU': result[6][0], 'cuquantum_CPU': result[6][1], 'tensordot_CPU': result[7][0], 'tensordot_GPU': result[7][1], 'einsum_CPU': result[8][0], 'einsum_GPU': result[8][1], 'correctness': result[9], 'fastest_CPU': result[10], 'fastest_GPU': result[11], 'speedup_CPU': result[12][0], 'speedup_GPU': result[12][1], 'theory_mem': result[13]}, ignore_index = True)
