...
class ProfileMultiple:
    ...
    def export(self) -> None:
        ...
        for result in self.results:
            print(len(result))
            df = df._append({'con_label': result[0], 'default_CPU': result[1][0], 'default_GPU': result[1][1], 'ttgt_CPU': result[2][0], 'ttgt_GPU': result[2][1], 'tgett_CPU': result[3][0], 'tgett_GPU': result[3][1], 'gett_CPU': i[4][0], 'gett_GPU': result[4][1], 'default_patient_CPU': result[5][0], 'default_patient_GPU': result[5][1], 'tensordot_CPU': result[6][0], 'tensordot_GPU': result[6][1], 'correctness': result[7], 'fastest_CPU': result[8], 'fastest_GPU': result[9]}, ignore_index = True)
